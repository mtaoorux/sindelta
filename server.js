...
app.use(cors({
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'auth-key', 'client-service', 'user-Id', 'user-id']
}));
app.options('*', cors());
...
// ═══════════════════════════════════════════════════════════
// BATCH DETAIL — returns full batch info with contents
...
app.get("/folder_contents", async (req, res) => {
  try {
    ...
    if (!course_id || !folder_id) {
      return res.status(400).json({ error: "Missing required query params: course_id and folder_id" });
    }

    const targetUrl =
      `https://vibrantacademykotaapi.akamai.net.in/get/folder_contentsv3` +
      `?course_id=${encodeURIComponent(course_id)}` +
      `&parent_id=${encodeURIComponent(folder_id)}&windowsapp=false&start=0`;
...
app.get("/video_details", async (req, res) => {
  try {
    const { course_id, video_id, class: cls, ytflag = "0", folder_wise_course = "1", lc_app_api_url = "", decrypt } = req.query;
    if (!course_id || !video_id) {
      return res.status(400).json({ error: "Missing required query params: course_id and video_id" });
    }
