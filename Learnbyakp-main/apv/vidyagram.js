...
async function loadFolder(folderId) {
  ...
  try {
    const data = await fetchJson(`${BASE_URL}/api/folder-contents?course_id=${encodeURIComponent(courseId)}&parent_id=${encodeURIComponent(folderId)}`);
    if (data.status === 200) {
...
async function loadInitialContent() {
  ...
  try {
    const data = await fetchJson(`${BASE_URL}/api/folder-contents?course_id=${encodeURIComponent(courseId)}&parent_id=-1`);
...
async function loadLiveData() {
  ...
  try {
    const data = await fetchJson(`${BASE_URL}/api/live-courses?course_id=${encodeURIComponent(courseId)}`);
    if (data.status === 200) {
...
async function loadPreviousLives() {
  ...
  try {
    const data = await fetchJson(`${BASE_URL}/api/previous-live-videos?course_id=${encodeURIComponent(courseId)}`);
    if (data.status === 200) {
...
async function openContentItem(item) {
  ...
  if (item.material_type === "VIDEO") {
    ...
    window.location.href = `vgplayer?course_id=${encodeURIComponent(courseId)}&video_id=${encodeURIComponent(item.id)}`;
    return;
  }
...
function handleLiveClick(event) {
  ...
  if (type === "watch-previous" || type === "previous") {
    window.location.href = `vgvideo?course_id=${encodeURIComponent(courseId)}&video_id=${encodeURIComponent(id)}`;
    return;
  }

  if (type === "watch-live" || type === "live") {
    window.location.href = `vgvideo?course_id=${encodeURIComponent(courseId)}&video_id=${encodeURIComponent(id)}&isLive=true`;
  }
}
